...
!     Neutral Code:
!     1= H2,2= HE, 3= H, 4= CH4, 5= C2H2, 6= C2H4, 7= C2H6, 8= C4H2, 9= H2vib
!

!     Ion Code:
!     1=H+, 2=He+, 3=H2+, 4=H3+, 5=HeH+, 6=CH5+, 7=CH4+, 8=CH3+, 9=CH2+
!     10=CH+, 11=C+, 12=C2H5+, 13=C2H4+, 14=C2H3+, 15=C2H2+, 16=C3H7+,
!     17=C3H5+, 18=C3H4+ 19=C3H2+, 20=C3H+, 21=C3H3+, 22=C2H6+,
!	  23=C2H7+, 24=CnHm+, 25=C3H6+, 26=H2C3H+, 27C2H+.
...
character*1 Blank
parameter(NIons=16,NNeut=9,NGrid=200,NReact=52,Blank=' ')
...
real,dimension(NGrid) :: Alt,Rad,DE,TEGr
...
write(AtmosFil,"('./input/mars_neutdens.dat')") !change here JW
...
write(PhIonFil,"('./output/primprod.dat')")  !int(CHI)
open(unit=35,file=PhIonFil,status='unknown')
!* Ion production from secondary electron impact
! write(EImpPrFil,"('./common/output/',I0,'/secprod.dat')") int(CHI) !?????? comment out JW
! open(unit=40,file=EImpPrFil,status='unknown')
!* Electron temp is now apart of the atmosphere file
! write(TElecFil,"('./common/input/Tempjup.txt')")
...
write(50,*)'! Temperatures used = ',trim(AtmosFil)
6000 format(1x,A,A,3x,A,I2,A,I2,A,I4)
6001 format(1x,A,A,13x,A,I2,A,I2,A,F4.1)

! read in the electron TEMPERATURE profile. SJH - now done with atmosphere file
! Discard first 4 lines of comments and labels.  Rad is dummy.
! do i=1,Ngrid
! 	read(45,*) rad(i),TeGr(i)
! end do

!JW temperature at 1000
do i=1,Ngrid
 	TeGr(i)=1000 !just equate these to 1000?
end do
...
do  I=1,NGRID !Assuming a well mixed atmosphere
	read(30,*) Rad(I),DN(I,4), DN(I,1), DN(I,2), DN(I,3), DN(I,8) !JW changed Rad(i) to Alt(i)
end do
Rad=Rad*1e5
...
! Then n = n_0 * exp(-z/H)
do I=1,NGrid !!!   need 5-9
	! if(I .lt. 17) then
		DN(I,5) = DN(I,1)* 1E-5 !H2 from Haider et al., 2011
		DN(I,6) = DN(I,1)* 1E-10 !H
		DN(I,7) = DN(I,1)* 1E-2 !Ar from Haider et al., 2011
		DN(I,9) = DN(I,1)* 1E-7 !H2O from Haider et al., 2011
! 	elseif(I .ge. 17)then
! ! scale heights
...
do  I=NGrid,1,-1 !Assuming a well mixed atmosphere
	write(123,*) Rad(I), DN(I,1),DN(I,2),DN(I,3),DN(I,4),DN(I,5),DN(I,6),DN(I,7),DN(I,8),DN(I,9)
end do

do I = 1,NGRID
  write(57,7115) RAD(i)*1e-5, (DN(I,J),J=1,9)
	do o=1,9
		DNREF(I,o)=DN(I,o)
	end do
...
! read in REACTION LIST from file in the form Neutral X
! + Ion Y --> Ion Z
read(60,*)
read(60,*)
read(60,*)
...
elseif(pe .eq. 1)then
	...
	do I=1,5  !was 4, made 5 to handle ion names
		read(35,*)
	end do
	write(*,*) 'photoelectron input'
	do I=1,NGrid
		read(35,*)Rad(i),(PhIon(I,J),J=1,11)
	  if(PhIon(I,J) .eq. 0.0)then
	    PhIon(i,j) = 1e-20
	  endif
	end do
...
! Therefore this set of correlation relations is needed. 1,3,4,7,
		...
		!ProdPh(I,2) = PhIon(I,3) !+ 1*EImpPr(I,3)   He+ prod
		...
		!ProdPh(I,8) = PhIon(I,5) !+ EImpPr(I,5) !CH3+ prod
		!ProdPh(I,9) = PhIon(I,6) !+ EImpPr(I,6) !CH2+ prod
		!ProdPh(I,10) = PhIon(I,7) !+ EImpPr(I,7) !CH+ prod
		!ProdPh(I,11) = PhIon(I,8) !+ EImpPr(I,8) !C+ prod
...
7100 format(1x,'Diagnostics for Mars PCE on ',I2,'-',I2,'-',I4,' at ',I2,&
		 ...
		 ' km where the neutral densities are:',/,3x,'CO2',8x,'CO',7x,'O',8x,'N2', 8x,'H2', 8x, 'H', 8x, 'Ar', 8x, 'He', 8x, 'H20') !JOE WIMMERGREN need to these to the 9 Neutral's for Mars
	  	write(55,7110) (DN(ID,J),J=1,9) !JW changed from 1 to 9, and included the neutrals as well as 8x spaces between
